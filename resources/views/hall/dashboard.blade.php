            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <table class="min-w-full leading-normal">
                        ...
                        <tbody>
                            @foreach($halls->groupBy(function($item) { return $item->created_at->format('Y-m-d'); }) as $date => $groupedHalls)
                                <tr>
                                    <th colspan="3" class="text-lg font-semibold bg-gray-200 p-2">{{ $date }}</th>
                                </tr>
                                @foreach($groupedHalls as $hall)
                                    <tr>
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <p class="text-gray-900 whitespace-no-wrap">{{ $hall->group->name }}</p>
                                        </td>
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <p class="text-gray-900 whitespace-no-wrap">{{ $hall->reader->surname }} {{ $hall->reader->name }}</p>
                                        </td>
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <p class="text-gray-900 whitespace-no-wrap">{{ $hall->created_at->format('H:i') }}</p>
                                        </td>
                                    </tr>
                                @endforeach
                            @endforeach
